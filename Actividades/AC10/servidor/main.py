...
class Servidor:
    ...
    def enviar_estado(self, mensaje, continuar):
        ...
        stringified_value = str(mensaje)
        msg_bytes = stringified_value.encode()
        msg_length = len(msg_bytes).to_bytes(4, byteorder='big')
        mensaje_codificado = msg_length + msg_bytes
    ...
    def recibir_accion(self):
        ...
        while len(bytes_recibidos) < response_length:
            read_length = min(4096, response_length - len(bytes_recibidos))
            bytes_recibidos.extend(self.socket_cliente.recv(read_length))
        accion = bytes_recibidos.decode()        # Respuesta resultante
